...
def create_video(values):
    ...
    for row in sheet.iter_rows():
        for cell in row:
            ...
            print("\nGenerating tweet image for tweetID: {}".format(tweetId))
            tweetImgPath, text = download_tweet(tweetId)
            text = demoji.replace(text) # remove emojis
            text = re.sub(r"http\S+", "", text) # remove any http links

            if tweetImgPath != None:
                print("Tweet image successfully generated for tweetID: {}".format(tweetId))
                tweet_img_paths.append(tweetImgPath)
            else:
                print("Tweet image could not be generated for tweetID: {}".format(tweetId))

            print("\nGenerating TTS for tweetID: {}".format(tweetId))
            ...
            tweetAudioPath = download_tts(tweetId, text, voice_id=voice_id, speed=reading_speed,
                                          volume=reading_volume, pitch=reading_pitch)
            if tweetAudioPath != None:
                print("Tweet audio successfully generated for tweetID: {}".format(tweetId))
                tweet_audio_paths.append(tweetAudioPath)
            else:
                print("Tweet audio could not be generated for tweetID: {}".format(tweetId))
    ...
    intro_clip = resize(intro_clip, height=1080, width=1920)
    ...
    outro_clip = resize(outro_clip, height=1080, width=1920)

    bg_music_clip = AudioFileClip(values.get(BG_MUSIC))
    bg_music_clip = volumex(bg_music_clip, 0.2)
    ...
    actual_vid_duration = intro_dur + outro_dur + tweet_section_dur
...
layout = [ [sg.Text(text="Intro"), sg.FileBrowse(key=INTRO_PATH)],
           [sg.Text(text="Intro duration (s)"), sg.InputText(key=INTRO_DURATION, default_text="5")],
           [sg.Text(text="Outro"), sg.FileBrowse(key=OUTRO_PATH)],
           [sg.Text(text="Outro duration (s)"), sg.InputText(key=OUTRO_DURATION, default_text="5")],
